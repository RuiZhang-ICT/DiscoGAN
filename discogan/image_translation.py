...
def get_data():
    ...
    # rui add data reading
    else:
        dataA_dir = args.dataset_dir + args.dataA
        dataB_dir = args.dataset_dir + args.dataB
        testA_dir = args.dataset_dir + args.testA
        testB_dir = args.dataset_dir + args.testB
        dataA_imgs = os.listdir(dataA_dir)
        dataB_imgs = os.listdir(dataB_dir)
        testA_imgs = os.listdir(testA_dir)
        testB_imgs = os.listdir(testB_dir)
        data_A = [dataA_dir + f for f in dataA_imgs]
        data_B = [dataB_dir + f for f in dataB_imgs]
        test_A = [testA_dir + f for f in testA_imgs]
        test_B = [testB_dir + f for f in testB_imgs]
        test_A = test_A[:args.n_test]
        test_B = test_B[:args.n_test]
    # rui 

    return data_A, data_B, test_A, test_B

def get_fm_loss(real_feats, fake_feats, criterion, device): # rui
#def get_fm_loss(real_feats, fake_feats, criterion):
    losses = 0
    for real_feat, fake_feat in zip(real_feats, fake_feats):
        l2 = (real_feat.mean(0) - fake_feat.mean(0)) * (real_feat.mean(0) - fake_feat.mean(0))
        loss = criterion( l2, Variable( torch.ones( l2.size() ) ).to(device) ) # rui
        #loss = criterion( l2, Variable( torch.ones( l2.size() ) ).cuda() )
        losses += loss

    return losses

def get_gan_loss(dis_real, dis_fake, criterion, cuda, device): # rui
#def get_gan_loss(dis_real, dis_fake, criterion, cuda):
    ...
    labels_dis_real = Variable(torch.ones_like( dis_real ))
    labels_dis_fake = Variable(torch.zeros_like( dis_fake ))
    labels_gen = Variable(torch.ones_like( dis_fake ))
    ...
    if cuda:
        ...
        labels_dis_fake = labels_dis_fake.to(device)
...
def main():
    ...
    for epoch in range(epoch_size):
        ...
        for i in range(n_batches):
            ...
            dis_loss_A, gen_loss_A = get_gan_loss( A_dis_real, A_dis_fake, gan_criterion, cuda, device ) # rui
            fm_loss_A = get_fm_loss(A_feats_real, A_feats_fake, feat_criterion, device) # rui
            ...
            dis_loss_B, gen_loss_B = get_gan_loss( B_dis_real, B_dis_fake, gan_criterion, cuda, device ) # rui
            fm_loss_B = get_fm_loss( B_feats_real, B_feats_fake, feat_criterion, device ) # rui
